...
`
	...
	err := cleanupBuildkit(githubactions.New())
...
func TestCleanupBuildkitFailsAfterRemovingPreparedUnusedVolume(t *testing.T) {
	previousState, previousErr := os.ReadFile(buildkitPreparedStateFile())
	t.Cleanup(func() {
		if previousErr == nil {
			_ = os.WriteFile(buildkitPreparedStateFile(), previousState, 0o600)
...
`
	...
	err := cleanupBuildkit(githubactions.New())
	if err == nil || !strings.Contains(err.Error(), "cleanup=false") {
		t.Fatalf("prepared-only cleanup error = %v", err)
	}
	if _, err := os.Stat(buildkitPreparedStateFile()); !os.IsNotExist(err) {
