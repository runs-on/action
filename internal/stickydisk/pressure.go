...
func checkCritical(action *githubactions.Action, mountRoot string) {
	...
	if targets, err := activeCacheTargets(mountRoot); err != nil {
		action.Warningf("Sticky disk is critically full (%s), but active cache mounts could not be inspected: %v; skipping automatic reset.", stats, err)
		return
	} else if len(targets) > 0 {
		action.Warningf("Sticky disk is critically full (%s), but caches are mounted by an earlier action invocation; skipping automatic reset. Increase the sticky disk size.", stats)
		return
	}
