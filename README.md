...
The `git` mode accelerates `actions/checkout` without changing the checkout step. It starts a local Git proxy whose bare repository lives on the sticky disk, and rewrites `https://github.com/` fetch URLs to it (global `url.insteadOf`). The cache contains the complete history reachable from `GITHUB_SHA`, so `fetch-depth: 0` and deeper shallow checkouts work. GitHub still supplies the live ref advertisement. A requested branch, tag, SHA, or secondary repository outside the cache goes upstream unchanged instead of making the first checkout download every ref.
...
* Mirror syncs authenticate with the `token` input (default: `${{ github.token }}`). In `git-full` mode, checking out **other private repositories** requires passing a PAT with access to them, since the rewritten URLs no longer match the credentials configured by `actions/checkout`.
