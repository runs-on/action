...
func cacheMount(action *githubactions.Action, mountRoot, target string, rootOwned bool) (hit bool, err error) {
	...
	if dirNonEmpty(target) {
		if err := mergeTargetIntoCache(action, target, src, false); err != nil {
			...
			// Cold sources are removed by the deferred cleanup.
			if hit {
				if cleanupErr := removeCacheDir(action, src); cleanupErr != nil {
	...
	if fi, statErr := os.Lstat(target); statErr == nil {
		if fi.Mode()&(os.ModeSymlink|os.ModeIrregular) != 0 {
			...
			// Runners are ephemeral and every failure path here restores or
			// removes its backup, so anything already at the backup path is
			// user or checkout content — refuse to destroy it.
			if _, err := os.Lstat(backup); err == nil {
				return hit, fmt.Errorf("backup path %s already exists and was not created by this action; move it aside or choose a different cache path", backup)
	...
	if err := runMklinkJunction(action, target, src); err != nil {
...
// runMklinkJunction is an indirection point so unit tests can simulate
// junction-creation failure deterministically: PATH-based stubbing of cmd
// proved unreliable across Windows images (Defender quarantines freshly
// written executables, and batch-file interception did not take effect).
var runMklinkJunction = func(action *githubactions.Action, target, src string) error {
	return runLogged(action, "cmd", "/c", "mklink", "/J", target, src)
}
